...
def load_translation_model(config: ModelConfig = default_config):
    ...
    if not os.path.exists(model_path):
        error_msg = f"MarianMT model not found at {model_path}. Please download it first."
...
def load_whisper_model(config: ModelConfig = default_config) -> WhisperModel:
    ...
    logger.info(f"Loading Faster Whisper model '{config.whisper_model_name}' from {model_path}")
    
    if not os.path.exists(model_path):
        error_msg = f"Faster Whisper model not found at {model_path}. Please download it first."
        logger.error(error_msg)
        raise FileNotFoundError(error_msg)
    ...
    try:
        device = config.device or ("cuda" if torch.cuda.is_available() else "cpu")
        logger.info(f"Using device: {device}")
        logger.debug(f"Model configuration: compute_type={config.whisper_compute_type}, "
