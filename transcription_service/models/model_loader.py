...
def load_translation_model():
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"MarianMT model not found at {model_path}. Please download it first.")
...
def load_whisper_model(model_name: str) -> WhisperModel:
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Faster Whisper model not found at {model_path}. Please download it first.")
    print(f"Loading Faster Whisper model '{model_name}' from {model_path}...")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    return WhisperModel(model_path, device=device, compute_type="int8", cpu_threads=2, num_workers=5)